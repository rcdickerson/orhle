...
eliminatesAtLeastOne states assertion = case states of
  ...
  state:rest -> do
    ...
    case steval of
      Rejected  -> pure True
      Accepted  -> eliminatesAtLeastOne rest assertion
...
statesMeeting assertion states = do
  ...
  let toBool predResult = case predResult of
                            Accepted -> True
                            Rejected -> False
                            Error err -> error err
  let statesList = (map fst) . (filter snd) $ map (\(st, result) -> (st, toBool result)) evaluations
