...
build tool. ORHLE is currently built using GHC 8.10.4; see
`stack.yaml` for the full dependency resolver. To build ORHLE, execute
`stack build` or `stack install` from the project's root directory.
The executable takes as an argument a single `.imp` file. You can
either run the executable generated by Stack directly, or use Stack to
execute ORHLE:
